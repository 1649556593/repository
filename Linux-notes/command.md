...
文件大小：以字节为单位
时间戳：访问时间（atime）、修改时间（mtime）、状态更改时间（ctime）
链接计数：指向该inode的文件名数量
数据块指针：指向文件实际数据在磁盘上的存储位置
inode不存储的内容
文件名：文件名存储在目录项（dentry）中，目录本质上是一个将文件名映射到inode编号的特殊文件
文件实际数据：文件的实际内容存储在数据块（block）中1
inode的主要作用
唯一标识文件：通过inode编号在文件系统中准确定位文件对象
支持硬链接：多个文件名可以指向同一个inode，共享相同的数据，只有当链接计数归零时，数据才会被真正删除
提高文件系统性能：修改文件属性或权限时只需更新inode，无需重写整个文件数据
资源限制管理：文件系统创建时会预分配inode表，inode总数在格式化时基本固定。当inode耗尽时，即使磁盘空间充足也无法创建新文件
文件访问流程
当你通过文件名访问一个文件时，系统的工作流程是：
通过目录项找到文件名对应的inode编号
通过inode编号获取inode信息
根据inode中的数据块指针读取文件的实际数据
常用查看命令
ls -i：查看文件的inode编号
stat：查看文件的详细inode信息
